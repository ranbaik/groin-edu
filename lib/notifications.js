...
<div style="max-width:520px;margin:0 auto;background:white;border-radius:12px;overflow:hidden">
  ...
  try {
    await sendEmail({ to: student.email, subject, html })
    await logNotification({ enrollmentId: enrollment.id, studentId: student.id, type: 'email', event: 'reminder', status: 'sent', recipient: student.email, subject })
    return { success: true }
  } catch (err) {
    await logNotification({ enrollmentId: enrollment.id, studentId: student.id, type: 'email', event: 'reminder', status: 'failed', recipient: student.email, errorMsg: err.message })
    return { success: false, error: err.message }
  }
}
...
export async function sendEnrollmentSMS({ student, course, enrollment }) {
  ...
  try {
    ...
    await logNotification({ enrollmentId: enrollment.id, studentId: student.id, type: 'sms', event: 'signup', status: 'sent', recipient: student.phone, body: text })
    return { success: true }
  } catch (err) {
    await logNotification({ enrollmentId: enrollment.id, studentId: student.id, type: 'sms', event: 'signup', status: 'failed', recipient: student.phone, errorMsg: err.message })
    return { success: false, error: err.message }
  }
}
